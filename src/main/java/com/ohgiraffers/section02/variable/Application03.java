...
public class Application03 {
    public static void main(String[] args) {
        ...
        /*필기
        *   변수의 명명 규칙
        *  변수의 이름을 지을 때 아무렇게나 짓는 것이 아닌 규칙이 있다.
        *  실무적으로는 굉장히 중요하기 때문에 반드시 숙지해야하고,
        *  또한, 규칙에 맞는 올바흔 변수명을 짓는 것이 좋은 프로그래밍을 하는 첫 단추이다.*/

        /*필기
        *   변수의 명명 규칙
        *  1. 컴파일 에러를 발생시키는 규칙 // 빨간불 뜨는 몇몇 변역안된 작업 ?
        *   1-1 동일한 범위 내에서 동일한 변수명을 가질 수 없다.
        *   1-2 예약어는 사용이 불가능하다. (예약어: 미리 작성되어진 언어. 시스템이랑 이렇게 하기로 한 약속)
        *   1-3 변수명은 대/소 문자를 구분한다.(자바는 대/소문자를 엄격하게 구분하는 언어이다)
        *   1-4 변수명은 숫자로 시작할 수 없다.(사작만 할 수 없고 숫자를 섞어서 사용하는 것은 가능.)
        *   1-5 특수기호는 '_', '$' 만 사용 가능하다.
        *  2. 컴파일 에러를 발생시키지는 않지만 개발자들끼리의 암묵적인 규칙
        *   2-1. 변수명의 길이 제한은 없다.(하지만 적당히)
        *   2-2. 변수명이 합성어로 이루어진 경우 첫 단어는 소문자, 두 번째 시작 단어는 대문자로 시작한다.(카멜 케이지)
 ...
 //       int 1age = 20;  // 숫자로 시작하면 오류가 발생한다
        ...
        *  목차. 2-1. 변수명의 길이 제한은 없다. 하지만 적당히*/
        int asdjasdlknasfljnaslfnaskdkadsdnkaksldasd ;


        /* 목차. 2-2 수명이 합성어로 이루어진 경우 첫 단어는 소문자, 두 번째 시작 단어는 대문자로 시작한다.*/
        ...
        /* 목차. 2-3. 단어와 단어 사이의 연결을 언더스코어(_)로 하지 않는다. (타 언어 네이밍 규칙이다)*/
        String user_name;
        String userName;
